...
export function inputElements(){
    ...
    const combinedList = arrTA.map((arrival, index) => ({
        arrivalTime: arrival,
        burstTime: burTA[index]
    }));

    if (burTA.some(isNaN) || arrTA.some(isNaN)) {
        submits.textContent = "Please enter valid numbers only.";
        throw new Error("Invalid input: Non-numeric values detected.");
    }
    if (arrTA.length === 0 || burTA.length === 0) {
    ...
    return { arrT, burT, arrTA, burTA, combinedList, outputProcess, outputs, submits };
}